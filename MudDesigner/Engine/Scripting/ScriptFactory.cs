...
namespace MudEngine.Engine.Scripting
{
    ...
    public static class ScriptFactory
    {
            ...
            try
            {
                foreach (var a in assemblyCollection.Where(a => a != null))
                {
                    ...
                    foreach (Type t in types)
                    {
                        ...
                        if (type != null)
                        {
                            // If the returned object is not null, then 't' inherits.
                            foundScript = true;
                            script = t;
                            break;
                        }
        ...
        public static Type[] GetTypesWithInterface<T>() where T : class
        {
            ...
            try
            {
                foreach (Assembly assembly in assemblyCollection)
                {
                    types.AddRange(assembly.GetTypes().Where(type => type.GetInterface(typeof(T).Name) != null).ToList());
                }
            }
            catch (Exception)
            {
